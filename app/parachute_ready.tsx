...
export default function ParachuteReadyScreen() {
    ...
    useEffect(() => {
        ...
        const subscribeAccelerometer = () => {
            // stream and round data to two decimals
    ...
    useEffect(() => {
        // fire springs automatically on check updates
        LayoutAnimation.configureNext(LayoutAnimation.Presets.spring);
    }, [completedTasks]);

    if (!fontsLoaded || loading) {
        // structural fallback shell to stop early layout popping
        return (
                        ...
                        >
                            {isStarting ? (
                                ...
                                <Text style={[styles.startChallengeText, trialCount >= 3 && { color: '#FFF' }]}>
                                    {trialCount >= 3 ? "Out of Trials" : "Start Drop Session"}
                                </Text>
                            )}
                        </TouchableOpacity>
                    )}
...
const styles = StyleSheet.create({
    ...
    disabledTrialBtn: { backgroundColor: '#FF3B30', borderColor: '#D32F2F', borderWidth: 1.5 },
